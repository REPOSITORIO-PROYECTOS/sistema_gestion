    ...
    public Mono<Student> registerStudent(UserInfo userDetails, String username) {
        return userService.getFullName(username)
                .flatMap(name -> {
                    ...
                    student.setCoursesIds(userDetails.getStudent().getCoursesIds());
        ...
        return userService.getFullName(username)
            .flatMap(name -> studentRepository.findByEmail(username)
                ...
                .flatMap(existingStudent -> {
                    ...
                    existingStudent.setCoursesIds(userDetails.getStudent().getCoursesIds() != null && !userDetails.getStudent().getCoursesIds().isEmpty() ? userDetails.getStudent().getCoursesIds() : existingStudent.getCoursesIds());
